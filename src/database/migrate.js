...
function normalizeFlags(flags) {
  ...
  flags.env = flags.env || process.env.KNEX_ENV || process.env.NODE_ENV || 'upgrade';
}
...
function knexInit(flags) {
  ...
  flags.migrations = flags.migrations || (config.migrations && config.migrations.directory) || 'database/migrations';
  flags.migrations = join(rootDir, flags.migrations);
...
function umzugKnex(flags, connection) {
  return new Umzug({
    storage: join(__dirname, 'migration-storage'),
...
async function applyStepOption(command, umzug, opts, steps) {
  ...
  let migrations = command === 'up' ? await umzug.pending() : await umzug.executed().then(m => m.reverse());
